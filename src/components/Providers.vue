...
export default {
    ...
    methods: {
        ...
        canShowCart(cart) {
            return cart && cart.select && cart.select.request && cart.select.request.message && cart.select.request.message.order;
        },
        cartTotal(cart) {
            let totalAmount = 0;
            let currency = undefined;
            cart.select.request.message.order.items.forEach(item => {
                totalAmount += (1.0 * item.quantity.selected.count * item.price.value)
            ...
            <div v-if="canShowCart(cart)" class="w-full border-b-2 border-b-black">
                ...
                <div class="mt-1 px-2 flex font-bold w-full">
                    <div class="w-3/4 pt-2">
                        {{ cart.select.request.message.order.provider.descriptor.name }}/{{
                            cart.select.request.message.order.provider.locations[0].descriptor.name
                        }}
                    </div>
                    <div class="w-1/4">
                        <Button @click="onCheckOutCart($event, cart)" class="w-full h-full font-extrabold">Book</Button>
                    </div>
                </div>
                    ...
                    <div class="flex">
                        <div class="w-2/3 mt-1">
                            ...
                            <div class="w-full">
                                <Badge class="bg-blue-500 text-xs m-1 px-2" v-for="(pid, pi) in item.payment_ids">
                                    {{ pid }}&nbsp;<i class="fa-solid fa-check"></i>
                                </Badge>
                            </div>
                            <div class="w-full">
                        ...
                        <div class="w-1/3 mt-1">
                            ...
                            <div class="w-full flex  justify-end ">
                                <img style="height: 5rem;" v-bind:src="item.descriptor.images[0].url" />
                            </div>
                        </div>
                    </div>
